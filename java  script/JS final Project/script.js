  ...
  },
  {
    ...
    function loadCart() {
      ...
      if (saved) cardlist = JSON.parse(saved);
    }
    ...
    function displayproduct() {
      products.forEach(product => {
        ...
        box.append(img, title, desc, price, btn);
        mainbox.appendChild(box);
      });
    }

    function addToCart(product) {
      const exist = cardlist.find(item => item.id === product.id);
      if (exist) {
        exist.quantity += 1;
      } else {
        product.quantity = 1;
    ...
    function displayCart() {
      ...
      cardlist.forEach((item, index) => {
        ...
        cartItem.append(img, detail, removebtn);
      ...
      totalDiv.style.marginbuttom = "auto";
